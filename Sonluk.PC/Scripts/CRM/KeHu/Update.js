        ...
        success: function (resdata) {
            ...
            table.render({
                ...
                cols: [[ //表头
                  ...
                  { field: 'DWDZMS', title: '定位地址描述', width: 250 },
                  { field: 'SFDES', title: '省份', width: 90 },
                  { field: 'CSDES', title: '城市', width: 90 },
                  { field: 'JD', title: '经度', width: 150 },
                  { field: 'ED', title: '纬度', width: 150 },
    ...
    $.ajax({
        ...
        success: function (resdata) {
            ...
            table.render({
                ...
                cols: [[ //表头
                 //{ title: '序号', templet: '#indexTpl', width: 60 },
                 { field: 'XSMC', title: '累计送货次数', width: 120 },
                 { field: 'XSSL', title: '单次送货数量(只)', width: 150 },
...
//资质数据加载，顺带判断有哪些对应的细项需要显示
function TableLoad_zz(khid, khlx) {
    var table = layui.table;
    $.ajax({
        type: "POST",
        //async: false,
        ...
        success: function (resdata) {
            ...
            if (khlx == 5 || khlx == 10) {
                table.render({
                    ...
                    cols: [[ //表头
                     { title: '序号', templet: '#indexTpl', width: 60 },
                     { field: 'ZZMCIDDES', title: '资质名称', width: 120 },
                     { field: 'INFO1', title: '信息描述', width: 120 },
                     { fixed: 'right', width: 160, align: 'center', toolbar: '#tb_area' }
                    ]]
                });
            }
            else if (khlx == 1 || khlx == 10) {
                table.render({
                    elem: '#tb_zz',
                    data: data,
                    page: true, //开启分页
    ...
    $.ajax({
        ...
        success: function (resdata) {
            ...
            table.render({
                ...
                cols: [[ //表头
                 { title: '序号', templet: '#indexTpl', width: 60 },
                 { field: 'HDMCDES', title: '活动名称', width: 180 },
...
$(document).ready(function () {
    ...
    table.render({
        ...
        cols: [[ //表头
         ...
         { field: 'BEIZ', title: '备注', width: 200 },
         { field: 'SFZYLXR', title: '是否主要联系人', width: 120, templet: '#major_Tpl' },
         { title: '操作', fixed: 'right', width: 150, align: 'center', toolbar: '#tb_contact' }
        ]]
    });
    ...
    //if (model.ISACTIVE == 2 && sessionStorage.getItem("check") == 1) {
    //    $("#btn_save_only_check").show();
    //}
    ...
    if (typevalue == 1 || typevalue == 2 || typevalue == 4) {   //$("#Insert_KHtype").val("经销商");
        ...
        if (model.MCSX == 1) {
            ...
            $("#city2p").val(model.CS);
            getDropDownData(34, model.CS, "county2p");
            $("#county2p").val(model.COUNTY);
    ...
    $("#insert_display").click(function () {
        ...
        layer.open({
            ...
            success: function (index, layero) {
                //if (typevalue == 5) {
                //    $("#div_displayitem").show();
                //}
                //else {
                //    $("#div_displayitem").hide();
                //}
            },
            yes: function (index, layero) {
    ...
    $("#insert_qiandaoDZ").click(function () {
        ...
        layer.open({
            ...
            yes: function (index, layero) {
                ...
                if ((dz.split('市').length <= 2 && (dz.indexOf("市") < 0 || dz.indexOf("县") < 0) && (dz.indexOf("市") < 0 || dz.indexOf("区") < 0) && (dz.indexOf("市") < 0 || dz.indexOf("镇") < 0)) && dz.indexOf("自治") < 0 ) {
                    layer.msg("地址信息不正确！");
                    return false;
                }
    ...
    $("#btn_save_only_check").click(function () {
                    ...
                    success: function (result) {
                        ...
                        layer.open({
                            ...
                            yes: function (index, layero) {
                                if (res.KEY > 0)
                                    //  window.location.reload();
                                    window.location.href = "../KeHu/Select_LKA_SH";
                                layer.close(index);
                            },
                            end: function (index, layero) {
    ...
    $("#btn_save_only_back").click(function () {
    ...
    layui.use(['form', 'layer', 'element', 'table', 'laydate', 'upload'], function () {
        ...
        form.on('select(shuliang_type14)', function (data) {
            switch (data.value) {
                case "792":
                    $("#shuliang_year14").val(year - 1);
                    break;
                case "793":
                    $("#shuliang_year14").val(year);
                    break;
                default:
    ...
    if (sessionStorage.getItem("justwatch") == "1") {
        ...
        $("#temp").append(
         ...
         + '<a class="layui-btn layui-btn-xs" lay-event="watch">查看</a></script>'
    ...
    //LKA审核页面专用
    if (sessionStorage.getItem("justwatch") == "3" && sessionStorage.getItem("check") == "1" && model.ISACTIVE == 2) {
        // $("button").show();
        $("#btn_save_only").hide();
        $("#btn_save_only_check").show();
        $("#btn_save_only_back").show();
