    ...
    layui.use(['form', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        upload.render({
            ...
            before: function () {
                ...
                TableLoad_fujian($("#LKATSCLMXID").val(), 33, "table_fujian", "附件名称");
            },
            error: function () {
                //请求异常回调
                //layer.msg("上传失败");
        ...
        table.on('tool(result_mx)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (obj.event == 'edit') {
                ...
                layer.open({
                    ...
                    success: function () {
                        if (data.ISACTIVE != 10) {
                            TableLoad_opinion_watch(data.LKATSCLMXID, 1361, 'tb_opinion');
                        }
                        else {
                            TableLoad_opinion(data.LKATSCLMXID, 1361, 'tb_opinion');
                        }
                    ...
                    yes: function (index, layero) {
                        ...
                        $.ajax({
                            ...
                            success: function (result) {
                                var res = JSON.parse(result);
                                if (res.KEY > 0) {
                                    TableLoad_opinion($("#LKATSCLMXID").val(), 1361, 'tb_opinion');
                                    layer.close(index);
                                }
