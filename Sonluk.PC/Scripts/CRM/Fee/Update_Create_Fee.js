$(document).ready(function () {
    ...
    if (sessionStorage.getItem("TTID") != null && sessionStorage.getItem("TTID") != "") {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                //if (res.ISACTIVE >= 30 && sessionStorage.getItem("zzfwatch") == 1) {
                //    Hide_btn();
                ...
                //  //  layer.msg("显示附件：广告完成画面");
                //}
                if (res.ISACTIVE == 40 && sessionStorage.getItem("zzfwatch") == 1) {
                    Hide_btn();
                    $("#div_finshad").show();
                    $("#div_button").show();
                    $("#btn_pass").show();
                    $("#div_f").addClass("layui-show");
                ...
                + '<script type="text/html" id="tb_fujian2">'
                   + '<a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="delete">删除</a>'
                     + '<a class="layui-btn layui-btn-xs" lay-event="watch">查看</a>'
                    + ' <a class="layui-btn layui-btn-xs" lay-event="text">审核</a>'
                + '</script>'
                     );

                }
                if (res.ISACTIVE == 60 && sessionStorage.getItem("zzfwatch") == 1) {
                    Hide_btn();
    ...
    TableLoad_wjjl_Finsh(TTID, 36, "finshad", "广告完成画面照片");//加载广告完成画面照片
    ...
    $("#btn_submit_OA").click(function () {
        ...
        if (!xx.test($("#CLFY").val()) && $("#CLFY").val() != "") {
            ...
            var data = {
                ...
                SL: $("#SL").val(),
                GGWSJE: $("#GGWSJE").val() == "" ? 0 : $("#GGWSJE").val(),
                HTYEAR: $("#HTYEAR").val(),
            }
        ...
        if (obj.event == 'edit') {
            ...
            layer.open({
                ...
                success: function (layero, index) {
                    $("#div_opinion_content2").val(data.HFNR);
                    form.render();
                },
                yes: function (index, layero) {
    ...
    table.on('tool(finshad)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            layer.open({
                ...
                yes: function (index, layero) {
                    ...
                    $.ajax({
                        ...
                        success: function (id) {
                            if (id > 0) {
                                TableLoad_wjjl_Finsh(TTID, 36, "finshad", "广告完成画面照片");
                                layer.msg("删除成功！");
                            }
                            else
                                layer.msg("删除失败");
        ...
        else if (obj.event == 'text')
        {
          var lay = layer.open({
                ...
                moveOut: true,
                success: function (layero, index) {
                    $("#sp_altitude").val(data.SPZT);
                    $("#div_opinion_content").val(data.OPINION);
                    form.render();
                },
                yes: function () {
                    data.SPZT = $("#sp_altitude").val();
                    data.OPINION = $("#div_opinion_content").val();
                    $.ajax({
                        type: "POST",
                        async: false,
                        url: "../Fee/Update_File",
        ...
        success: function (resdata) {
            ...
            table.render({
              ...
              //   { field: 'TTID', title: '制作费申请抬头', width: 160 },
                 { field: 'CHANG', title: '长（米）', width: 110 },
                 { field: 'KUAN', title: '宽（米）', width: 110 },
        ...
        success: function (resdata) {
            ...
            table.render({
             ...
             //    { field: 'TTID', title: '制作费申请抬头', width: 140},
                 ...
                 { field: 'PRICE', title: '单价（未税,元）', width: 150 },
                 { field: 'QUANTITY', title: '数量', width: 130 },
                 { field: 'AMOUNT', title: '金额（未税,元）', width: 150, totalRow: true },
                 { field: 'BEIZ', title: '备注', width: 150 },
        ...
        success: function (resdata) {
            ...
            table.render({
                ...
                cols: [[ //表头
                 ...
                 { width: 180, align: 'center', toolbar: '#tb_fujian2',  title: "操作" }
                ]]
            });
            $("img.mytableimg").parent().css("height", "auto");
        },
