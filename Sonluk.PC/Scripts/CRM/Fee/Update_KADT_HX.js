    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for (var i = 0; i < MXdata.length; i++) {
                ...
                if (MXdata[0].ISACTIVE == 30 || MXdata[0].ISACTIVE == 40 || MXdata[0].ISACTIVE == 45) {
                    $("#btn_submit").show();
                    ...
                    $("#temp").append('<script type="text/html" id="bar_fujian_watch">'
            ...
            table.render({
                ...
                cols: [[ //表头
                    ...
                    { field: 'COSTITEMIDDES', title: '费用类型', width: 120 },
                    ...
                    { field: 'SJFB', title: '实际费比', width: 100, templet: '#tpl_sjfb' },
                    { field: 'HDJSZJ', title: '活动结束总结', width: 120 },
                    { field: 'BEIZ2', title: '备注', width: 100 },
...
$(document).ready(function () {
    ...
    TableLoad();
    TableLoad_dp();
    TableLoad_fujian_watch($("#KADTTTID").val(), 26, "table_fujian", "附件名称");
    TableLoad_fujian($("#KADTTTID").val(), 41, "table_fujian_xp", "附件名称");
    ...
    $("#fy_sjxs").change(function () {
        if ($("#fy_sjxs").val() != "" && $("#fy_sjfy").val() != "" && $("#fy_sjxs").val() != 0) {
            $("#fy_fb").val(parseFloat(($("#fy_sjfy").val()) / parseFloat($("#fy_sjxs").val()) * 100).toFixed(2) + "%");
        }
    });
    $("#fy_sjfy").change(function () {
        if ($("#fy_sjxs").val() != "" && $("#fy_sjfy").val() != "" && $("#fy_sjxs").val() != 0) {
            $("#fy_fb").val(parseFloat(($("#fy_sjfy").val()) / parseFloat($("#fy_sjxs").val()) * 100).toFixed(2) + "%");
        }
    });
    ...
    layui.use(['form', 'layer', 'element', 'laydate', 'upload'], function () {
            ...
            if (obj.event == 'edit') {
                ...
                layer.open({
                    ...
                    success: function () {
                        ...
                        $("#fy_sjxs").val(data.SJXS);
                        $("#fy_sjfy").val(data.SJFY);
                        $("#fy_hdjszj").val(data.HDJSZJ);
                        $("#fy_beiz2").val(data.BEIZ2);
                        if (data.SJXS == 0)
                            $("#fy_fb").val("0");
                        else
                            $("#fy_fb").val(parseFloat(($("#fy_sjfy").val()) / parseFloat($("#fy_sjxs").val()) * 100).toFixed(2) + "%");
                    ...
                    yes: function (index, layero) {
                        ...
                        var reg = /^[0-9]+([.]{1}[0-9]{1,2})?$/;
                        if (!reg.test($("#fy_sjxs").val())) {
                            layer.msg("实际销售格式不正确，金额保留两位小数");
                            return false;
                        }
                        if (!reg.test($("#fy_sjfy").val())) {
                ...
                layer.open({
                    ...
                    //btn: ['确定', '取消'],
                    success: function () {
                        TableLoad_opinion(data.KADTMXID, 4120, 'tb_opinion');
                        form.render();
                    },
                    end: function () {
                    ...
                    yes: function (index, layero) {
                        ...
                        $.ajax({
                            ...
                            success: function (result) {
                                var res = JSON.parse(result);
                                if (res.KEY > 0) {
                                    TableLoad_opinion(data.OACSBH, 4120, 'tb_opinion');
                                    layer.close(index);
                                }
        ...
        table.on('tool(table_fujian_xp)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            var data = obj.data; //获得当前行数据
            var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
            var tr = obj.tr; //获得当前行 tr 的DOM对象
