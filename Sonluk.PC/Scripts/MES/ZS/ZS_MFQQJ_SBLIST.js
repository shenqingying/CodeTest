...
$(document).ready(function () {
    layui.use(['form', 'laydate', 'layer', 'element', 'table'], function () {
        ...
        setInterval(function () { Select_Data(isfree); }, 60000);
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (res.MES_RETURN.TYPE === "S") {
                ...
                $("#table").append(tr);
                var T_count = res.MES_ZS_QJ_QJSB.length;
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (res.MES_RETURN.TYPE === "S") {
                ...
                $("#div_result").append('<div class="layui-form" id="div_test"><table id = "table"  class="cd-value" style="color:white;text-align:center;border-collapse:separate; border-spacing:10px;float:top" width="100%" >'
                    + '<tr id = "div_tr" height = "100%" ><td  colspan="3" style="text-align:left;">'
                    + ' <div class="layui-form"><div class="layui-input-inline">'
                    + ' <input type="radio" name="isfree" title="" value="2" lay-filter="isfree" checked >'
                    + ' <lable style= "color:red" id="lable_machine">机器数：' + res.MES_ZS_QJ_QJSB.length + '</lable>&nbsp'
                    + ' <input type="radio" name="isfree" title="" value="0" lay-filter="isfree">'
                    + ' <lable style="color:green"id="lable_online">全检中：' + Temp_OnLine + '</lable>&nbsp'
                    + ' <input type="radio" name="isfree" title="" value="3" lay-filter="isfree">'
                    + ' <lable style= "color:Orange"id="lable_back">返工中：' + Temp_BACK + '</lable>&nbsp'
                    + ' <input type="radio" name="isfree" title="" value="1" lay-filter="isfree">'
...
function showTime() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            //  console.log(res);
            console.log(result);
            document.getElementById("div_time").innerText = "系统时间：" + result;
