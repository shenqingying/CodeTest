...
layui.use(['form', 'laydate', 'element', 'laydate', 'table'], function () {
    ...
    $("#btn_add").click(function () {
        layer.open({
            ...
            success: function (layero, index) {
                isadd = 1;
                $("#clpbdm_gc").removeAttr("disabled");
                $("#clpbdm_ycldm").removeAttr("disabled");
                $("#clpbdm_pbdm").removeAttr("disabled");
                claer_div_clpbdm();
                band_table_tb_clpbdm_wl([]);
                form.render();
            },
            yes: function (index, layero) {
                var GC = $("#clpbdm_gc").val();
                var YCLDMID = $("#clpbdm_ycldm").val();
                var PBDMID = $("#clpbdm_pbdm").val();
                var YCLPBDM = $("#clpbdm_clpbdm").val();
                var REMARK = $("#clpbdm_remark").val();
                var datastring1 = table.cache.tb_clpbdm_wl;
                if (GC === "") {
                    layer.alert("请选择工厂");
                    return;
                }
                else if (YCLDMID === "0" || YCLDMID === "") {
                    layer.alert("请选择原材料代码");
                    return;
                }
                else if (PBDMID === "0" || PBDMID === "") {
                    layer.alert("请选择配料代码");
                    return;
                }
                else if (datastring1.length === 0) {
                    layer.alert("下层组件不能为空");
                    return;
                }
                else {
                    var datastring = {
        ...
        if (layEvent === 'modify') {
            layer.open({
                ...
                success: function (layero, index) {
                    ...
                    $("#clpbdm_ycldm").val(dataline.YCLDMID);
                    $("#clpbdm_pbdm").val(dataline.PBDMID);
...
function band_table_tb_clpbdm_wl(data) {
    ...
    table.render({
        ...
        cols: [[ //表头
            ...
            { fixed: 'right', width: 100, align: 'center', toolbar: '#barkh1', title: '操作' }
        ]]
        , page: false
    });
}
