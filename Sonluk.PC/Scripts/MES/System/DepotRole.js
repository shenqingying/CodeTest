layui.use(['form', 'laydate', 'element','laydate', 'table'], function () {

    var curpage = 1;
    var c = 1;

    function TableLoad() {
        var table = layui.table,
            form = layui.form

        var GC = $('#cx_gc').val();
        $.ajax({
            type: "POST",
            async: false,
            url: "../System/Data_Select_CK",
            data: {
                GC: GC
            },
            success: function (data) {
                var resdata = JSON.parse(data);
                if (resdata.MES_RETURN.TYPE == "S") {
                    table.render({
                        done: function (res, curr, count) {
                            c = count;// - 1;

                            if (curr > Math.ceil(c / 15)) {
                                curpage = Math.ceil(c / 15);
                            }
                            else { curpage = curr; }
                            return curpage;
                        },
                        elem: '#tb_ck',
                        page: {
                            limits: [15],
                            limit: 15,
                            curr: curpage
                        }, //开启分页
                        data: resdata.MES_ROLE_ASSEMBLE,
                        cols: [[ //表头
                            { title: '序号', templet: '#indexTpl', width: 60 },
                            { field: 'GC', title: '工厂', width: 110 },
                            { field: 'ROLENAME', title: '仓库名称', width: 150 },
                            { fixed: 'right', width: 120, align: 'center', toolbar: '#bar' }
                        ]]
                    });
                }
                else {
                    layer.msg(resdata.MES_RETURN.MESSAGE);

                }

            },
        });
    }

    $("#btn_cx").click(function () {
        $(".layui-laypage-skip .layui-input").val(1);//指定某页
        $(".layui-laypage-skip .layui-laypage-btn").click();//刷新

        TableLoad()
    })

    $(document).ready(function () {

        var table = layui.table;
        var form = layui.form;
        var element = layui.element;
        var layer = layui.layer;
         $("#gc_xg").attr("disabled", "disabled");
        TableLoad()

    $("#btn_insert").click(function () {

        layer.open({
            type: 1,
            shade: 0,
            area: ['400px', '350px'], //宽高
            content: $('#div_insert'),
            btn: ['保存', '取消'],
            title: '新增数据信息',
            moveOut: true,
            yes: function (index, layero) {

                if ($("#ckname").val() == "") {
                    layer.msg("仓库名称不可为空！");
                    return false;
                }

                if ($("#gc").val() == "") {
                    layer.msg("请输入工厂！");
                    return false;
                }

                var newdata = {
                    GC: $("#gc").val(),
                    ROLENAME: $("#ckname").val()
                };

                $.ajax({
                    type: "POST",
                    async: false,
                    url: "../System/Data_Insert_CK",
                    data: {
                        data: JSON.stringify(newdata)
                    },
                    success: function (addlist) {
                        var i = JSON.parse(addlist);
                        if (i.TYPE == "S") {
                            layer.msg("新增成功！");
                            TableLoad();


                        }
                        else {
                            layer.msg(i.MESSAGE);
                        }


                    },
                    error: function () {
                        alert("操作失败，请联系管理员");
                    }
                });

                layer.close(index);
            },
            end: function () {
                $("#gc").val(""),
                $("#ckname").val(""),


                 $('#div_insert').hide();

                form.render();
            }
        });
    });

    $("#btn_sap").click(function () {
        if ($("#cx_gc").val() == "") {
            //layer.msg("请输入“工厂”进行SAP同步！");
            layer.tips('请在此输入“工厂”进行ERP同步！', $('#cx_gc').parent());
            return false;
        }

        var GC = $('#cx_gc').val();

        layer.open({
            title: '提示',
            type: 0,
            content: '确认开始同步吗？',
            btn: ['确定', '取消'],
            yes: function (index, layero) {
                $.ajax({
                    type: "POST",
                    async: false,
                    url: "../System/GET_SAP_CK",
                    data: {
                        GC: GC,
                    },
                    success: function (data) {
                        var i = JSON.parse(data);
                        if (i.TYPE == "S") {
                            layer.msg("同步成功！");
                            TableLoad();


                        }
                        else {
                            layer.msg(i.MESSAGE);
                        }


                    },
                    error: function () {
                        alert("同步失败，请联系管理员");
                    }
                });
            }
        })
    });

    form.on('select(cx_gc)', function (data) {
        $.ajax({
            type: "POST",
            async: false,
            url: "../System/Data_Select_GC",
            data: {},
            success: function (reslist) {
                var res = JSON.parse(reslist);
                for (var i = 0; i < res.length; i++) {
                    if (res[i].GC == $('#cx_gc').val()) {
                        if (res[i].ISAUTOWORKSPACE == false) {
                            $("#btn_sap").hide();
                            layer.tips('该工厂同步方式为手动，不可进行ERP同步！', $('#cx_gc').parent());
                            //layer.msg("该工厂同步方式为手动，不可进行SAP同步！")
                        } else { $("#btn_sap").show(); }
                    }
                }
                form.render();
            }
        });
    });

    table.on('tool(tb_ck)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        var data = obj.data; //获得当前行数据
        var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
        var tr = obj.tr; //获得当前行 tr 的DOM对象

        if (layEvent == "edit") {

            layer.open({
                type: 1,
                shade: 0,
                btn: ['保存', '取消'],
                area: ['450px', '500px'],
                content: $('#div_ck'),
                title: '仓库修改维护',
                moveOut: true,
                success: function (layero, index) {
                    $("#gc_xg").val(data.GC);
                    $("#ckname_xg").val(data.ROLENAME);
                    form.render();
                    $.ajax({
                        type: "POST",
                        async: false,
                        url: "../System/Data_Select_CKROLE",
                        data: {
                            ROLEID: data.ID
                        },
                        success: function (data) {
                            var resdata = JSON.parse(data);
                            if (resdata.MES_RETURN.TYPE = "S") {
                                table.render({
                                    elem: '#result_ck',
                                    id: 'result_ck',
                                    data: resdata.MES_ROLE_CK_LAY,
                                    limit: 10000,
                                    width: 430,
                                    //height: 400,
                                    cols: [[
                                            { type: 'checkbox' },
                                            { title: '序号', templet: '#indexTpl', width: 60 },
                                            { field: 'CKDM', title: '仓库代码', width: 100 },
                                            { field: 'CKMS', title: '仓库明细', width: 220 }

                                    ]],
                                    page: false

                                });
                            }
                            else {
                                layer.msg(resdata.MES_RETURN.MESSAGE);
                            }

                        }
                    });
                },

                yes: function (index, layero) {
                    if ($("#ckname_xg").val() == "") {
                        layer.msg("仓库名称不可为空！");
                        return false;
                    }
                    var newdata = {
                        ID: data.ID,
                        GC: data.GC,
                        ROLENAME: $("#ckname_xg").val()
                    };
                    var rolecheck = table.checkStatus('result_ck');
                    $.ajax({
                        type: "POST",
                        async: false,
                        url: "../System/Data_Update_CK",
                        data: {
                            data:JSON.stringify(newdata)
                        },
                        success: function (list) {
                            var res = JSON.parse(list);
                            $.ajax({
                                type: "POST",
                                async: false,
                                url: "../System/Data_Update_FPCK",
                                data: {
                                    ROLEID: data.ID,
                                    ckdata: JSON.stringify(rolecheck.data)
                                },
                                success: function (data) {
                                    var resdata = JSON.parse(data);
                                    if (resdata.TYPE == "S" && res.TYPE=="S") {
                                        layer.msg("仓库修改维护成功！");
                                        TableLoad();
                                    }
                                    else if(resdata.TYPE == "E"){
                                        layer.msg(resdata.MESSAGE);
                                    }
                                    else if (res.TYPE == "E") {
                                        layer.msg(res.MESSAGE);
                                    }
                                }
                            });
                        }
                    })
                    layer.close(index);
                },

                end: function () {
                    $("#div_ck").hide();
                }
            })
        }
        else if (layEvent == "delete") {
            layer.open({
                title: '提示',
                type: 0,
                content: '确定删除?',
                btn: ['确定', '取消'],
                yes: function (index, layero) {

                    $.ajax({
                        type: "POST",
                        async: false,
                        url: "../System/Data_Delete_CK",
                        data: {
                            ID: data.ID
                        },
                        success: function (id) {
                            var i = JSON.parse(id);
                            if (i.TYPE == "S") {
                                layer.msg("删除成功！");
                                var dc = 1;
                                var ds = c - 1;
                                if (curpage > Math.ceil(ds / 15)) {
                                    dc = Math.ceil(ds / 15);
                                    $(".layui-laypage-skip .layui-input").val(dc);//指定某页
                                    $(".layui-laypage-skip .layui-laypage-btn").click();//刷新
                                    TableLoad();
                                }
                                else {
                                    dc = curpage;
                                    $(".layui-laypage-skip .layui-input").val(dc);//指定某页
                                    $(".layui-laypage-skip .layui-laypage-btn").click();//刷新
                                    TableLoad();
                                }
                            }
                            else {
                                layer.msg(i.MESSAGE);
                            }
                        },
                        error: function (err) {
                            layer.msg("系统错误,请重试！");
                        }
                    });
                    layer.close(index);
                }
            });
        }
    });

});
})