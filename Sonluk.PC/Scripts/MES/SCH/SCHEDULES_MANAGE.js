...
layui.use(['form', 'layer', 'element', 'table', 'laydate'], function () {
    ...
    document.getElementById("btn3").onclick = function (data) {
        var checkStatus = table.checkStatus('tb1');
        var data = checkStatus.data;
        if (data.length == 0) {
            layer.msg(sonluklv.alert14);
        } else {
            ...
            var index1 = layer.open({
                ...
                maxmin: true,
                area: ['350px', '200px'],
                title: sonluklv.open3,
                content: $('#div5'),
                moveOut: true,
                btn: [sonluklv.open7, sonluklv.open8],
                success: function (layero, index) {
                    layero.style.zIndex = 99999;
                    $$('div5_pbrq').value = getNowFormatDay(1);
                },
                yes: function (index, layero) {
                    var requestData = [];
                    var pbdate = $$('div5_pbrq').value;
                    for (var i = 0; i < data.length; i++) {
                        var node = {
                            GDDH: data[i].GDDH,
                            ZPRQ: pbdate,
    ...
    //            shade: 0,
    //            area: ['350px', '200px'],
    //            title: sonluklv.open4,
    //            content: $('#div6'),
    //            moveOut: true,
    //            btn: [sonluklv.open7, sonluklv.open8],
    //            success: function (layero, index) {

    //                $$('div6_pbrq').value = getNowFormatDay(1);
    //            },
    //            yes: function (index, layero) {
    //                var requestData = [];
    //                var pbdate = $$('div6_pbrq').value;
    //                if (pbdate == '') {
    //                    layer.msg(sonluklv.alert17);
    //                    return;
    //                }
    ...
    $$('btn5').onclick = function (data) {
        layer.open({
            ...
            success: function (layero, index) {
                ...
                $$('div7_pbrq').value = getNowFormatDay(1);
                document.querySelectorAll('.input_content').forEach(function (value, index) {
                    value.value = '';
                });

                var isauto = sonluk.save.get('div7_pbswitch') == null ? 'checked' : sonluk.save.get('div7_pbswitch');//|| true;//document.getElementsByName('div7_pbswitch')[0].checked;
                var isyzbb = sonluk.save.get('div7_yzmanual') == null ? 'checked' : sonluk.save.get('div7_yzmanual');//|| true;//document.getElementsByName('div7_yzmanual')[0].checked;

                document.getElementsByName('div7_pbswitch')[0].checked = isauto;

                document.getElementsByName('div7_yzmanual')[0].checked = isyzbb;
            ...
            yes: function (layero, index) {
                ...
                if (gddh == '') {
                    layer.msg(sonluklv.alert20);
                    return;
                }
                if (erpno == '') {
                    layer.msg(sonluklv.alert21);
                    return;
                }
    ...
    $$('btn7').onclick = function (data) {
        layer.open({
            ...
            yes: function (a, b) {
                //var gddh = $$('div8_gddh').value;
                //var erpno = $$('div8_erpno').value;
                var status = $$('div8_status').value;
                if (status == '') {
                    layer.msg(sonluklv.alert22);
                    return;
                }
                var tablebase = layui.table.cache.tb5;
                if (tablebase.length == 0) {
                    layer.msg(sonluklv.alert31);
                    return;
                }
                var data = [];
                for (var i = 0; i < tablebase.length; i++) {
                    data.push({
                        ERPNO: tablebase[i].ERPNO,
                        GDDH: tablebase[i].GDDH,
                        STATUSMS: status
                    })
                }
                $.ajax({
                    type: 'Post',
                    url: $('#BatchModifyGD_SCH_STATUS').val(),
                    data: {
                    ...
                    success: function (data) {
                        data = JSON.parse(data);
                        if (data.type == 'S') {
                            layer.msg(sonluklv.alert32);

                        } else {
                            layer.msg(sonluklv.alert33);
                        }
                        LoadRequestTB1(true);
                        //$$('div8_tm').focus();
                        //document.querySelectorAll('.input_content').forEach(function (value, index) {
    ...
    //        a.innerHTML = data;
    //        $('#div_img').append(a);
    //        html2canvas(document.getElementById('div_img')).then(function (canvas) {
    //            document.querySelector('#id_img').src = canvas.toDataURL(1);
    //            $('#div_img').empty();
    //        });
    //    }
    //})
                        ...
                        if (isauto == true) {
                            ...
                            if (isyzbb == true) {
                                if (gddh != '' && erpno != '' && manualbb != '') {
                                    ScanPBRequest(gddh, erpno);
                                } else if (manualbb == '' && gddh != '' && erpno != '') {

                                    layer.confirm(sonluklv.alert34, {
                                        btn: [sonluklv.open7, sonluklv.open8] //按钮
                                    }, function () {
                                        ScanPBRequest(gddh, erpno);
                                    }, function () {
...
function LoadTable2(data) {
    ...
    var form = layui.form;
    //for (var i = 0; i < data.length; i++) {
    //    if (data[i].MATNR_POS.substr(0,1) == '3') {
    //        data[i].TMSL = 10000;
    //    }
    //    //var a = data[i].MATNR_POS.substr(0, 1);
    //    //var c = 2;
    //}
    ...
    layer.open({
        ...
        success: function (layero, index) {
            ...
            stable.render({
                ...
                cols: [[ //表头
                    ...
                    { field: 'MATNR_POS', width: 120, fixed: 'left', templet: '#matnr_pos' },// templet: '#templet_wcsl'  
                    { field: 'MAKTX_POS', width: 250, align: 'left' },
                    { field: 'TMSL', width: 130 },
        ...
        yes: function (index, layero) {
            var checkStatus = layui.table.checkStatus('tb2');
            var data = checkStatus.data;
            if (data.length == 0) {
                layer.msg(sonluklv.alert9);
                return;
