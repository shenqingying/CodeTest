...
{
    public partial class frmTL2_2 : baseview
    {
        #region property
        string _lastDate;
        ...
        /// <param name="shebeihaoID"></param>
        /// <param name="rtype"></param>
        public frmTL2_2(MES_PD_SCRW_LIST list, string shebeihaoID,Rigth_Type rtype)
        {
            InitializeComponent();
        ...
        {
            string[] cols = { q(Msg_Type.fieldxh), q(Msg_Type.fieldtm), q(Msg_Type.fieldwlxx), q(Msg_Type.fieldwllb), q(Msg_Type.fielddelete), q(Msg_Type.fieldwllbdm), q(Msg_Type.fieldtmdm), q(Msg_Type.fieldgc), q(Msg_Type.fieldtltmdm), q(Msg_Type.fieldwlh) };
            //"序号", "条码", "物料信息", "物料类别", "删除", "物料类别代码", "条码代码", "工厂","投料条码代码","物料号"
            string[] colss = { "Xh", "Tm", "Wlxx", "Wllb", "del", "Wllbdm", "tmid", "gc","tltmid","wlh" };
            string[] vcols = { q(Msg_Type.fieldwllbdm), q(Msg_Type.fieldtmdm), q(Msg_Type.fieldgc), q(Msg_Type.fieldtltmdm),q(Msg_Type.fieldwlh) };
            //{ "物料类别代码", "条码代码", "工厂" ,"投料条码代码","物料号"};
        ...
        public MsgReturn VerifyWLisALL()
        {
            ...
            {
                if (list1.Exists(x=>x.IDNRK == BomList.ET_BOM[i].IDNRK) == false)
                {
        ...
        private void getTMList(SELECT_MES_TM_TMINFO_BYTM model,int first)
        {
            for (int x = 0; x < model.MES_TM_TMINFO_LIST.Length; x++)
            {
                ...
                node.Wlh = model.MES_TM_TMINFO_LIST[x].WLH;
                ...
                for (int i = 0; i < _bomList.ET_BOM.Length; i++)
                {
                    ...
                    //else
                    //{
                      if (_bomList.ET_BOM[i].IDNRK == node.Wlh)
                        {
                            isBom = true;
                ...
                if (isBom == false)
                {
                    if (MessageBox.Show(q(Msg_Type.msgbominvalid), q(Msg_Type.msgtitle), MessageBoxButtons.OK) == DialogResult.OK)//"你扫描的条码不是物料的BOM组成部分！！"
                    {
                        SMtextBox.Clear();
                        SMtextBox.Select();
                    }
                ...
                if (Tmlist.Count > 0)
                {
                    ...
                    for (int i = 0; i < Tmlist.Count; i++)
                    {
                        if (node.Tm == Tmlist[i].Tm)
                        {
                            //replace = true;
                            //Tmlist[i] = node;
                            if (MessageBox.Show(string.Format(q(Msg_Type.msgtmcf), node.Tm), q(Msg_Type.msgtitle), MessageBoxButtons.OK) == DialogResult.OK)//node.Tm + "已经存在不能重复投料"
                            {
                                SMtextBox.Clear();
                                SMtextBox.Select();
                            }
                    ...
                    //{
                        ...
                        for (int i = 0; i < _bomList.ET_BOM.Length; i++)
                        {
                            for (int j = 0; j < Tmlist.Count; j++)
                            {
                                ...
                                //else
                                //{
                                if (_bomList.ET_BOM[i].IDNRK == Tmlist[j].Wlh)
                                {
                                    BOMdataGridView.Rows[i].Cells[q(Msg_Type.fieldstatus)].Style.BackColor = Color.FromArgb(187, 255, 102);//"状态"
                                }
                                //}
        ...
        {
            ZBCFUN_GDJGXX_READ r = ServicModel.PD_GD.SAP_GET_GDJGXX(MES_PD_SCRW_LIST.RWBH, dateTimePicker1.Text.Trim(), MES_PD_SCRW_LIST.GC, getToken());
            if (!r.MES_RETURN.TYPE.Equals("S"))
            {
                ShowMeg(r.MES_RETURN.MESSAGE);
                SMtextBox.Select();
        ...
        {
            //if (dateTimePicker1.Text.Trim().Equals(DateTime.Now.ToString("yyyy-MM-dd")))
            //{
            //     MES_PD_SCRW model = new MES_PD_SCRW();
            //    model.SBBH = _MES_PD_SCRW_LIST.SBBH;
            //    model.ZPRQ = dateTimePicker1.Text.Trim();
