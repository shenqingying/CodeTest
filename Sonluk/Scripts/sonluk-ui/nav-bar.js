$(document).ready(function () {
    ...
    $(document).on("click", "div.nav-bar-collapse", function () {

        $(".nav-bar").animate({ left: -205 }, "fast");
        $(".page").animate({ left: 0 }, "fast");
        $(this).animate({ left: 0 }, "fast");
        $(this).removeClass("nav-bar-collapse");
        $(this).addClass("nav-bar-expand");
        $(this).children().removeClass("nav-bar-display-arrow-left");
        $(this).children().addClass("nav-bar-display-arrow-right");
    })

    $(document).on("click", "div.nav-bar-expand", function () {

        $(".nav-bar").animate({ left: 0 }, "fast");
        $(".page").animate({ left: 205 }, "fast");
        $(this).animate({ left: 205 }, "fast");
        $(this).removeClass("nav-bar-expand");
        $(this).addClass("nav-bar-collapse");
        $(this).children().removeClass("nav-bar-display-arrow-right");
        $(this).children().addClass("nav-bar-display-arrow-left");
    })

    $(".sign-out").click(function () {

        //var uri = $(this).next().val();
        messageDialogWarning('退出登录?', function () {
            //var url = uri + '?LOGINTYPE=1';
            var content = $('#it_logonut').val();
            var url = content.substring(0, content.length - 1) + '?LOGINTYPE=1';
            location.href = url;
        });
    });
});
...
AccordionMenu.prototype = {
    ...
    _createSubMenu: function (Children, lihtml) {
        ...
        $(Children).each(function (index, item) {
            ...
            subLi = $('<li><a class="nav-bar-node" id="' + uri.replace(/[^a-zA-Z0-9]*/g, "") + '" href="' + uri + '">' + item.Node + '</a></li>');
            if (item.Children && item.Children.length > 0) {
    ...
    */
    _LoadStatus: function () {
        var id = this._GetCookie();
        var node = $("a#" + id);
        //alert(id);
        if (node.length > 0) {
            do {
                node.prev().children("img").addClass("unfold");
                node.show();
