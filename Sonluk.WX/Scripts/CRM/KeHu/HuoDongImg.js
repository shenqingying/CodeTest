...
//表格数据加载
function TableLoad_huodongimg(dxid, model) {
    $("#div_result").empty();
    var data;
                    ...
                    layer.open({
                        ...
                        yes: function (index, layero) {
                            ...
                            $.ajax({
                                ...
                                success: function (id) {
                                    if (id > 0) {
                                        TableLoad_huodongimg(dxid, model);
                                        layer.msg("删除成功！");
                                    }
                                    else
                                        layer.msg("删除失败");
...
$(document).ready(function () {
    ...
    var huodongID;
    if (sessionStorage.getItem("HuoDongID") == null) {
        alert("获取客户信息失败，请重试");
    ...
    //$("#btn_upload_huodong").click(function () {
    //    sessionStorage.setItem("model", JSON.stringify(model));
    //    ImgUpload(appid, state, 8, huodongID, ['album', 'camera'],0);
    //});

    $("#btn_upload_img_camera").click(function () {
        sessionStorage.setItem("model", JSON.stringify(model));
        ImgUpload(appid, state, 8, huodongID, ['camera'], 0);
    });

    $("#btn_upload_img_album").click(function () {
        sessionStorage.setItem("model", JSON.stringify(model));
        ImgUpload(appid, state, 8, huodongID, ['album'], 0);
    });
