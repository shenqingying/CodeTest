        ...
        success: function (resdata) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                $("#div_result").append('<table border="0" width="100%"><tr><td width="350" colspan="2">序号：' + xuhao + '</td></tr>'      //<td width="60"><button id="btn_edit' + xuhao + '" class="layui-btn layui-btn-xs" style="width:100%;height:30px">编辑</button></td>
                    + '<tr><td>年份：' + data[i].GSRQ + '</td><td width="150">数量：' + data[i].XSSL + '</td></tr>'
                    + '<tr><td colspan="2">信息类别：' + data[i].XSMC + '</td><td><button id="btn_delete' + xuhao + '" class="layui-btn layui-btn-xs layui-btn-danger" style="width:100%;height:30px">删除</button></td></tr>'
                    + '</table>');
...
$(document).ready(function () {
    ...
    for (var i = 3; i >= 0 ; i--) {
        $("#shuliang_year14").append("<option value='" + (year - i) + "'>" + (year - i) + "</option>");
        //$("#shuliang_year14").val(year);
    }

    $("#shuliang_type14").change(function () {
        switch ($("#shuliang_type14").val()) {
            case "792":      //网点数量
    ...
    $("#btn_save_WDshuliang").click(function () {
        $("#btn_save_WDshuliang").attr("disabled", "disabled");
        var mydate = new Date();
        var layer = layui.layer;
        if ($("#shuliang_year14").val() == "" || $("#shuliang_type14").val() == "0" || $("#shuliang_counts14").val() == "") {
            layer.msg("请输入网点数量信息");
            $("#btn_save_WDshuliang").removeAttr("disabled");
            return false;
        }
    ...
    $("#btn_nosave_WDshuliang").click(function () {
        $("#htitle").text("新增网点数量");
        $("#shuliang_type14").val("0");
        $("#shuliang_year14").val("");
        $("#shuliang_counts14").val("");
