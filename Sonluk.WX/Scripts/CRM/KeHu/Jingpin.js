...
//渠道表格数据加载
function TableLoad_jingpin(khid) {
    $("#div_result").empty();
    var data;
    ...
    $.ajax({
        ...
        success: function (resdata) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                $("#div_result").append('<table border="0" width="90%"><tr><td width="60">序号：' + xuhao + '</td><td>竞品名称：' + data[i].XXMC + '</td><td width="60"><button id="btn_delete' + xuhao + '" class="layui-btn layui-btn-xs layui-btn-danger" style="width:100%;height:30px">删除</button></td></tr></table>');
...
$(document).ready(function () {
    ...
    $("#btn_save_jingpin").click(function () {
        var mydate = new Date();
        var layer = layui.layer;
        $("#btn_save_jingpin").attr("disabled", "disabled");
        if ($("#jingpin005").val() == "0") {
            layer.msg("请选择竞品名称");
            $("#btn_save_jingpin").removeAttr("disabled");
            return false;
        }
        ...
        var QTdata = {
            KHID: khid,
            XXLB: 3,
            XXMC: $("#jingpin005").find("option:selected").text(),
            //操作人
